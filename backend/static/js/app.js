  ...
  if (feed) {
    ...
    feed.innerHTML = `
      <div class="flex items-start space-x-2.5">
        <div class="w-7 h-7 rounded-full bg-[#1976D2] flex items-center justify-center text-white flex-shrink-0 text-xs shadow-2xs">
...
function toggleStt() {
  ...
  if (!isListeningSTT) {
    try {
      ...
      speechRecognizer.onstart = () => {
        isListeningSTT = true;
        if (btn) {
          btn.classList.remove('bg-slate-50', 'hover:bg-slate-100', 'text-slate-600', 'border-slate-200');
          btn.classList.add('bg-red-100', 'border-red-500', 'text-red-600', 'animate-pulse');
      ...
      speechRecognizer.onerror = (e) => { 
        ...
        if (e.error !== 'no-speech') {
           alert('Microphone error: ' + e.error + '. Please check permissions.');
        }
        stopSTT(); 
      };
      ...
      console.warn('STT Setup Error:', err);
      alert('Could not start speech recognition. Error: ' + err.message);
      stopSTT();
    }
  } else {
    stopSTT();
  ...
  function stopSTT() {
    ...
    if (btn) {
      btn.classList.remove('bg-red-100', 'border-red-500', 'text-red-600', 'animate-pulse');
      btn.classList.add('bg-slate-50', 'hover:bg-slate-100', 'text-slate-600', 'border-slate-200');
      btn.innerHTML = '<i class="fa-solid fa-microphone"></i>';
    }
...
function speakText(text) {
  ...
  try {
    ...
    const langPrefix = targetCode.substring(0, 2);
    
    // Prefer Google, Premium, Natural, or Microsoft voices
    let matchedVoice = voices.find(v => v.lang.startsWith(langPrefix) && (v.name.includes('Google') || v.name.includes('Premium') || v.name.includes('Natural') || v.name.includes('Microsoft')));
    
    // Fallback
    if (!matchedVoice) {
      matchedVoice = voices.find(v => v.lang.startsWith(langPrefix) || v.lang === targetCode);
    }
    
    if (matchedVoice) {
      utterance.voice = matchedVoice;
      console.log(`[TTS] Selected Voice: ${matchedVoice.name}`);
    }
...
// Multilingual Quick Presets (Trauma-Informed)
const PRESET_SCENARIOS = {
  hi: {
    critical_panic: 'मुझे इस समय असुरक्षित महसूस हो रहा है।',
    elevated_threat: 'मुझे किसी से बात करनी है, कृपया मेरी बात सुनें।',
    depressive_flat: 'मुझे समझ नहीं आ रहा कि मुझे क्या करना चाहिए।',
    neutral: 'मुझे कानूनी जानकारी या सहायता की आवश्यकता है।'
  },
  en: {
    critical_panic: 'I feel unsafe right now.',
    elevated_threat: 'I need someone to listen to me.',
    depressive_flat: 'I don\'t know what to do.',
    neutral: 'I need legal information and support.'
  },
  bn: {
    critical_panic: 'আমি এই মুহূর্তে অনিরাপদ বোধ করছি।',
    elevated_threat: 'আমার কথা শোনার জন্য কাউকে প্রয়োজন।',
    depressive_flat: 'আমি বুঝতে পারছি না আমার কী করা উচিত।',
    neutral: 'আমার আইনি তথ্য প্রয়োজন।'
  },
  ta: {
    critical_panic: 'நான் இப்போது பாதுகாப்பற்றதாக உணர்கிறேன்.',
    elevated_threat: 'யாராவது என் பேச்சைக் கேட்க வேண்டும்.',
    depressive_flat: 'என்ன செய்வது என்று எனக்குத் தெரியவில்லை.',
    neutral: 'எனக்கு சட்டத் தகவல் தேவை.'
  },
  te: {
    critical_panic: 'నేను ప్రస్తుతం అసురక్షితంగా భావిస్తున్నాను.',
    elevated_threat: 'నా మాట వినడానికి నాకు ఎవరైనా కావాలి.',
    depressive_flat: 'ఏం చేయాలో నాకు అర్థం కావడం లేదు.',
    neutral: 'నాకు చట్టపరమైన సమాచారం కావాలి.'
  }
};
...
function appendUserMessage(text) {
  ...
  userDiv.innerHTML = `
    ...
    <div class="w-8 h-8 rounded-full bg-slate-200 flex items-center justify-center text-slate-600 flex-shrink-0 text-xs">
...
function showTypingIndicator() {
  ...
  div.innerHTML = `
    <div class="w-8 h-8 rounded-full flex-shrink-0 animate-pulse">
      <img src="/static/img/ai-avatar.png" class="w-full h-full object-cover rounded-full shadow-sm" />
    </div>
    <div class="chat-bubble-bot flex items-center space-x-2 text-xs shadow-sm">
      ...
      <span class="italic text-[11px] text-slate-500 font-medium">${thinkingLabel}</span>
    </div>
  `;
...
async function uploadAudioCheckin(blob) {
  ...
  if (responseLengthSelect) {
    formData.append('response_length', responseLengthSelect.value);
  }
